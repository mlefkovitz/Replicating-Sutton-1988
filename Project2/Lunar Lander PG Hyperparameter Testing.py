...
def lunarLander(H, batch_size, learning_rate, gamma):
    # hyperparameters
    ...
    print('Hyperparameters H', H, ' batch size', batch_size, ' learning rate', learning_rate, ' gamma', gamma)
        ...
        if done: # an episode finished
            ...
            if np.max(running_reward_array) == hundred_running_reward:
                ...
                if episode_number > 100:
                    ...
                    model_iterator += math.floor((100 / batch_size)/2)
    ...
    # plot the scores
    titlehyperparams = ' H = ' + str(H) + ' batch size = ' + str(batch_size) + ' learning rate = ' + str(learning_rate) + ' gamma = ' + str(gamma)
    fig = plt.figure()
    plt.plot(np.arange(len(reward_array)), reward_array, linestyle="", marker="o")
    plt.ylabel('Score')
    plt.xlabel('Episode #')
    plt.title('Score per Training Episode')
    plt.savefig('Score per Training Episode PG' + titlehyperparams + '.png', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig('Score per Trail (trained agent) PG' + titlehyperparams + '.png', bbox_inches='tight')
    plt.show()

# # hyperparameters
# Hs = [10, 64] # number of hidden layer neurons
# batch_sizes = [1, 10, 25] # every how many episodes to do a param update?
# learning_rates = [0.7, 0.1, 1e-2, 1e-3, 1e-4]
# gammas = [.99, 0.9, 0.7, 0.1] # discount factor for reward

# hyperparameters
Hs = [10] # number of hidden layer neurons
batch_sizes = [10] # every how many episodes to do a param update?
learning_rates = [0.03, 1e-2, 6e-3]
gammas = [.995, .99, 0.95] # discount factor for reward

# Hs = [10] # number of hidden layer neurons
# batch_sizes = [10] # every how many episodes to do a param update?
# learning_rates = [1e-2]
# gammas = [0.9] # discount factor for reward

for i in range(len(Hs)):
    for j in range(len(batch_sizes)):
        for k in range(len(learning_rates)):
            for z in range(len(gammas)):
                lunarLander(Hs[i],batch_sizes[j],learning_rates[k],gammas[z])
